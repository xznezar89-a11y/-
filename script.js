...
function renderProducts() {
    ...
    if (products.length === 0) {
        ...
        // إضافة رسالة "للمزيد من الخطط" في النهاية
        const morePlansDiv = document.createElement('div');
        morePlansDiv.className = 'more-plans-cta reveal';
        morePlansDiv.style.width = '100%';
        morePlansDiv.style.textAlign = 'center';
        morePlansDiv.style.marginTop = '40px';
        morePlansDiv.style.padding = '30px';
        morePlansDiv.style.background = 'var(--glass-bg)';
        morePlansDiv.style.borderRadius = '20px';
        morePlansDiv.style.border = '1px solid var(--glass-border)';
        morePlansDiv.style.gridColumn = '1 / -1'; // يجعلها تأخذ العرض الكامل في الشبكة (Grid)
        
        morePlansDiv.innerHTML = `
            <h3 style="color: var(--text-primary); margin-bottom: 10px;">هل تبحث عن خدمات أخرى؟</h3>
            <p style="color: var(--text-secondary); margin-bottom: 20px;">لمزيد من الخطط والطلبات الخاصة، يرجى التواصل مع الدعم الفني أو الوكيل مباشرة.</p>
            <div style="display: flex; gap: 15px; justify-content: center; flex-wrap: wrap;">
                <a href="support.html" class="btn-primary" style="padding: 10px 25px;">الدعم الفني</a>
                <a href="agents.html" class="btn-primary" style="padding: 10px 25px; background: rgba(255,255,255,0.1);">وكلاء الشراء</a>
            </div>
        `;
        container.appendChild(morePlansDiv);
    }
}
